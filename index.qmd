...
* Both Google maps and Trip Advisor are principally English language platforms when it isn't spoken by the majority of the population. Upon a cursory inspection, there are very few reviews in Malay or with Malay words in Google (`r malay_pc_summary |> filter(source == "Google") |> pull(malay_pc)`%) and even less in Trip Advisor (`r malay_pc_summary |> filter(source == "TripAdvisor") |> pull(malay_pc)`%). This is extremely different from even urban demographics, though this skew is more understandable on Trip Advisor than on Google. 

* The price range is missing from this data. Price is one of the main topics mentioned in reviews. We will not be able to make comparisons if we do not even have a starting point. 

* These reviews were extracted three years ago: some of these restaurants have closed. Before we re-scrape the data and update these datasets, let's see what is of most in the existing data (which honestly was probably used to train a model to summarise reviews or write fake reviews). 
...
It really could be said that the default review rating is five stars. This means that the ceiling for restaurant quality on these platforms is particularly low, given that it is hard for me to believe that half of all reviews were due to a truly exceptional dining experience. **`r round(google_kl_cuisine |> filter(rating == 5) |> nrow() / nrow(google_kl_cuisine) * 100, 2)`%** of all ratings in the Google dataset for the Klang Valley are five stars; in Trip Advisor, it was **`r round(trip_kl_cuisine |> filter(rating == 5) |> nrow() / nrow(trip_kl_cuisine) * 100, 2)`%**. 
...
Given that reviews are limited to a one-to-five scale, there is not very much differentiation amongst how restaurants are rated. In the scatterplots below, we note restaurants that mostly receive five-star reviews in blue, and restaurants with mostly four-star reviews in green, with some outliers at the lower end of the scale. Let us take a closer look to see if mean rating is closely related to excellence. 
...
In spite of all this, I still haven't learnt anything that I instinctually did not know. In my day-to-day life, when engaging with online reviews, I'm still forced to engage with reviews as individual data points. 
